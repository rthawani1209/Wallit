...
def get_budget_progress(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
...
def set_budget(
    ...
    budget = (
        db.query(Budget)
        .filter(Budget.user_id == current_user.id, Budget.category_id == category_id)
        .first()
    )
    if budget:
        budget.monthly_limit = body.limit
    else:
        budget = Budget(user_id=current_user.id, category_id=category_id, monthly_limit=body.limit)
        db.add(budget)
    db.commit()
